...
export function useLineChart(canvasRef: RefObject<HTMLCanvasElement>, data: [number[]], generateSteps: (a: number, b: number, c: number) => number,
  headers: string[], sideHeaders: { header: string, color: string }[], title: string, subTitle: string,
  dimenstions: { height: number, width: number }) {
  useEffect(() => {
